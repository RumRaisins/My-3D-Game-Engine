...
namespace E3D {
	...
	ETank::ETank(const EString & name, const EString & meshName, EGameManager * gameMgr) :mGameMgr(gameMgr),
		mScene(gameMgr->getSceneManager()), mSpeed(0.75f), mFireDir(0, 0, -1), mLastFire(0), mBulletType(BULLET_ROCKET),
	...
	ETank::~ETank() {
		mScene->destoryMesh(mName + "_Body");
		mScene->destoryMesh(mName + "_Head");
	}
	void ETank::fire() {
	...
	EBool ETank::moveUp() {
		...
		EFloat xOff= 0, zOff = 0;  //Tank中心向两侧的偏移值

		EVector3D pos = getPosition();  //移动后位置
		if (degree < 5) {
