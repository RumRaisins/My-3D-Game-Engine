...
namespace E3D {
	...
	void EGraphics::clearBuffer(const EColor &c)
	{
		...
		//重置深度缓存
		//注意，这里memset只能对Int类型的数组进行初始化，所以直接使用了Int类型的
		//而没有使用float，应使用loat
		::memset(GZBuffer, 0, sizeof(EFloat)*SCREEN_WIDTH*SCREEN_HEIGHT);
	}
	//在当前缓冲区绘制一条线
	...
	//检测Z深度值
	EBool EGraphics::checkZ(EInt x, EInt y, EInt z) {
		//这里的Z应该使用float类型来存储，如果使用int类型，那么会导致深度丢失
		//产生错误的现象
	...
	void EGraphics::setPixel(EInt x, EInt y, const EColor &c) {
		//这里本来应该计算Z值，但为了避免对Image像素的读取，将Z检测分批了出来
	...
	//将已经绘制好的缓冲区较低交给Graphics在屏幕上绘制，并将当前缓冲区设置为另一个缓冲区
