...
namespace E3D {
	...
	EFrustum::EFrustum(CAMERA_TYPE mode,const EVector4D &pos,const EVector4D &dir,const EVector4D &target,EFloat nearZ,EFloat farZ,
		EFloat ffov, EFloat viewportWidth, EFloat viewportHeight) :camMode(mode),position(pos),direction(dir),camTarget(target),
		clip_z_near(nearZ),clip_z_far(farZ),fov(ffov),viewport_widght(viewportHeight),viewport_height(viewportHeight),
		camUp(EVector4D::UNIT_Y),camRight(EVector4D::UNIT_X),camLook(EVector4D::UNIT_Z),mWorldToCamera(EMatrix44::IDENTITY),mCameraToPerspective(EMatrix44::IDENTITY),
		mPerspectiveToScreen(EMatrix44::IDENTITY){
		viewport_center_X = (viewport_widght - 1)*0.5f;
		viewport_center_Y = (viewport_height - 1)*0.5f;

		aspect_ratio = viewport_widght / viewport_height;

		viewplane_widght = 2.0f;
		viewplane_height = 2.0f / aspect_ratio;
		EFloat tan_fov_div2 = tan(Degree_To_Radian(fov*0.5f));
		view_dist = 0.5f*viewplane_widght / tan_fov_div2;

		//设置裁剪平面
		//裁剪平面坐标是基于摄像机坐标的
		EVector4D point = EVector4D::ZERO;
		//右裁剪面
		EVector4D normalR = EVector4D(-view_dist, 0, viewplane_widght*0.5f);
		clip_plane_R = EPlane3D(point, normalR);
		//左裁剪面
		EVector4D normalL = EVector4D(view_dist, 0, viewplane_widght*0.5f);
