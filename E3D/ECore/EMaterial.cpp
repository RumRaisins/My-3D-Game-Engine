...
namespace E3D {
	...
	EColor EMaterial::gitPixel(EFloat u, EFloat v) {
		...
		EMaterial *GetMaterial(const EString &name) {
			...
			if (iter == GMaterials->end())return NULL;
		...
		void ParseMaterialScript(const EString &scriptName) {
			...
			if (in.good()) {
				...
				while (in.good()) {
					...
					if (!info.empty()) {
						//遇到“{”加1，遇到“}”减1，当brackets==0时，跳出当前材质
						EInt brackets = 0;
						...
						if (index == 0) {
							...
							while (brackets != 0) {
								...
								//漫反射
									...
									if (diffuseIndex == 0) {
										sscanf(info.c_str(), "%s%f%f%f", temp, &material->diffuse.r,
								...
								//贴图
									...
									//这里使用texture，后面保留一个空格，与texture_unit区别
									if (textureIndex == 0) {
