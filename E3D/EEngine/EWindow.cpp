...
namespace E3D {
	...
	EGameWindow::EGameWindow(const EString &windowTitle, HINSTANCE hInstance) :mWindowTitle(windowTitle),
		mHinstance(hInstance), mCanvasLinster(NULL), mQuite(false) {
	...
	void EGameWindow::setCanvasListener(ECanvas *canvasLinster) {
		mCanvasLinster = canvasLinster;
	}
	...
	void EGameWindow::onPaint(HDC hdc) {
		if (mCanvasLinster) {
			...
			mCanvasLinster->update();
			mCanvasLinster->onPaint();
	...
	void EGameWindow::removeInputLinstener(EInputListener *listener) {
